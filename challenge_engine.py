...
import model_loss_statistics as _loss_statistics
...
TARGET_ODDS_MIN = 2.0
TARGET_ODDS_MAX = 3.0
USER_RECORDED_QUOTE_SOURCE = "USER_RECORDED"
MAX_TICKET_LEGS = 3
...
PAIRED_LOSS_CONFIDENCE_Z = _loss_statistics.PAIRED_LOSS_CONFIDENCE_Z
...
CHALLENGE_LEG_ODDS_FLOOR = 1.25
MIN_CHALLENGE_LEG_ODDS = max(
    MINIMUM_RECOMMENDED_DECIMAL_ODDS,
    CHALLENGE_LEG_ODDS_FLOOR,
)
...
class ChallengeCandidate:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.prediction_version is None:
            payload.pop("prediction_version")
        if self.validation is not None and self.validation.prediction_version is None:
            payload["validation"].pop("prediction_version")
...
def _paired_loss_statistics(
    ...
    return _loss_statistics.paired_advantage_statistics(paired_advantages)


def _benjamini_hochberg_q_values(
    p_values: dict[str, float],
) -> dict[str, float]:
    """Preserve the existing whole-family BH policy through shared arithmetic."""
    return _loss_statistics.benjamini_hochberg_q_values(p_values)
...
def fixture_market_probabilities(
    ...
    *,
    ...
    # calibrated season/form values. Owning ORIGINALs always require all three.
    if _validation_only and original_capture is not None:
        raise ValueError("validation-only predictions cannot capture a full ORIGINAL")
    ...
    from football_joint_calibration import LAW_VERSION, RECIPE, calibrate_joint_distribution
    model["probabilities"] = {}
    ...
    def project_family(family, specs, family_model, means_suffix):
        ...
        for variant in model["probability_variants"]:
            ...
            raw_matrix = (score_matrix(*raw_means) if family == "goals"
                          else family_model[variant + "_matrix"])
            effective, diagnostics = calibrate_joint_distribution(raw_matrix, specs, calibration)
            family_model["raw_" + mean_key] = raw_means
            family_model[mean_key] = tuple(
                sum(cell[side] * mass for cell, mass in effective.items()) for side in (0, 1))
            if family != "goals":
                family_model[variant + "_matrix"] = effective
            raw_values.append(_market_probabilities(raw_matrix, specs))
            # Identity/failure paths return the exact raw law. Reuse its
            # marginals, avoiding a second full settlement scan in warmup.
            effective_values.append(_market_probabilities(effective, specs)
                if diagnostics["status"] == "projected" else raw_values[-1])
            records[variant] = {
                "raw_means": list(raw_means),
                "raw_cells": [[h, a, p] for (h, a), p in sorted(raw_matrix.items())],
                "effective_cells": [[h, a, p] for (h, a), p in sorted(effective.items())],
    ...
    project_family("goals", GOAL_MARKET_SPECS, model, "_lambdas")
    model["count_models"] = {}
    for family, specs in (
        ...
        project_family(family, specs, count_model, "_counts")
    if original_capture is not None:
        original_raw_probabilities = dict(model["raw_probabilities"])
...
def _walk_forward_market_records(
    ...
    records: dict[str, dict[str, list[Any]]] = {
        spec.key: {"probabilities": [], "outcomes": [], "baselines": [], "raw": [], "projection_success": []}
        ...
        for fixture in day_fixtures:
            ...
            for spec in MARKET_SPECS:
                ...
                if probability_values is None or outcome_value is None:
                    continue
                raw_probability = prediction["raw_probabilities"][spec.key][0]
...
def _validation_metrics_from_records(
    ...
    for spec in MARKET_SPECS:
        ...
        passed = (
            ...
            and all(records[spec.key].get("projection_success", []))
