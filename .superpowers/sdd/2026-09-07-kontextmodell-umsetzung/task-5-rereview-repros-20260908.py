...
Rereview source: 22164fdf577190fa17f9aeddebd1b810ca4c59a1.
Owned copy of the preserved original. Only R1 field assertions and the explicitly
approved mixed-scope rejection are adapted; original controls remain unchanged.
"""
...
def test_available_refs_exclude_nonconsumed_observations(tmp_path, case):
    ...
    if case == "unrelated_wrong_revision":
        with pytest.raises(ContextContractError, match="scope"):
            state(rows, **policy_options)
        return
    result = state(rows, **policy_options)
    assert result["state"] == "available"
    assert result["refs"] == sorted([current, ignored]), (case, result)
    assert result["usable_refs"] == [current], (case, result)
