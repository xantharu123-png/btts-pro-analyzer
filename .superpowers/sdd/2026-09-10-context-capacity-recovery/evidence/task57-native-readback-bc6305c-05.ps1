...
root = Path("/var/lib/betboy-context-chain-task57-bc6305c-05")
def identity(s):
...
names = ["plan.json", "b764d67226dfba5ab97281d19a7ef903b542f810e4b212591d772c3f63ad488f.jsonl"]
...
_, original_archive = read(Path("/tmp/betboy-context-qa.9xr68INa/task57-bc6305c-05.tar"), 64 * 1024**2)
_, original_catalogue = read(Path("/tmp/betboy-context-qa.9xr68INa/task57-bc6305c-05-catalogue.json"), 8 * 1024**2)
...
$task57ReadbackOut = [IO.File]::Open((Join-Path (Get-Location) '.pytest_tmp/task57-native-run-bc6305c-05-readback.json'), [IO.FileMode]::CreateNew, [IO.FileAccess]::Write, [IO.FileShare]::Read)
$task57ReadbackErr = [IO.File]::Open((Join-Path (Get-Location) '.pytest_tmp/task57-native-run-bc6305c-05-readback.stderr'), [IO.FileMode]::CreateNew, [IO.FileAccess]::Write, [IO.FileShare]::Read)
...
Get-Item -LiteralPath .pytest_tmp/task57-native-run-bc6305c-05-readback.json, .pytest_tmp/task57-native-run-bc6305c-05-readback.stderr | Select-Object Name,Length | ConvertTo-Json
Get-FileHash -LiteralPath .pytest_tmp/task57-native-run-bc6305c-05-readback.json -Algorithm SHA256 | ConvertTo-Json
Get-Content -LiteralPath .pytest_tmp/task57-native-run-bc6305c-05-readback.stderr
