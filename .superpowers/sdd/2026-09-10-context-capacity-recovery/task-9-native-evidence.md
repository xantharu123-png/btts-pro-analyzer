...
Controller read the full report and independently checked frozen paths/product
hashes. Exact-product focus1109passed/12skipped659.74s; final75-case module and
later generic-count strengthening separately passed. Independent scoped review
is APPROVED with no actionable findings:365focused and32inventory tests passed,
plus12new actual persisted ATP/WTA workload controls. Root read the complete
review. This is not a native performance or final whole-branch review result.

Exact Git archive `capacity-task6-1c65ada.tar.gz`:33907914bytes,832safe members,
102923720expanded bytes, SHA256
`a3df9e45e9cc0c067197716b44930d6e7eea77557296193e6dab5ca011847e00`.
Git PAX revision and safe member paths/types/budgets validated locally. The
new exact target did not exist under the UID1000/mode0700 approved QA parent.
Approved transfer to `/tmp/betboy-context-qa.9xr68INa` completed and the remote
SHA matches. The unchanged root-stdlib stager (SHA0c0427f2...f9359) produced
`/var/lib/betboy-capacity-code-iym7re_h/source` and sibling synthetic fixtures.
All832source members are root-sealed; no app code was imported as root.

The exact uninstrumented current-input run is now active asUID997, child203242,
against that source and the253333504-byte saved actual copy below. Supervisor
`.pytest_tmp/measure_task9_profiles.py` SHA256
`b0d6eafe099ce177291ea5e55d18a380064eaa2c39c2b9e3e001f9de3ef5da1f`.
No result yet. No other heavy native QA job is running, and no production
source, data, service, updater or resource limit changed. The immutable archive
is distinct from the subsequent controller-documentation-only checkpoint.

## Pending implementation checkpoint — 11 September 2026

This is the controller's acceptance checklist, not a successful test report.
Implementation base: `d138ac5b14e94ac37a6b089bf3efbda3a7dc0c92`.
The Task9 implementer owns its five named production files, assigned tests,
report and Git index. No Task9 source has been committed or executed on the
VPS at this checkpoint. The complete Task8 current-input failure remains in
`task-8-native-evidence.md`; it is not superseded by unit-test success.

Required execution order:

1. Exact implementation commit, RED/GREEN report and independent scoped review.
2. Exact Git archive and SHA, root-sealed new source stage in the already
   authorized isolated QA workflow; no local secrets or runtime databases.
3. Uninstrumented exact CLI on the latest saved real sealed input FIRST.
   Failure stops release acceptance, regardless of focused test results.
4. New G1/G2/G3 receipt-plus-consumer fixtures AND retained historical largest;
   fresh full repository suite in the clean exact-LF QA worktree, and actual
   Linux UID997 permission/lifecycle controls. No parallel heavy native jobs.
5. Final whole-branch review of one immutable release candidate; carry the
   Task5 Minor unordered-SQL-test comparison finding into that review.
6. Fresh complete runtime backup, actual restore/HMAC, exact current-data
   verification, and unchanged protected-source/installer/updater pins.
7. Normal main fast-forward/push, reviewed updater-only installation, then
   separate normal exact-commit application update and post-release checks.

Fixed native contract: UID997, AS2GiB, CPU300, wall600, output1MiB,
single-threaded numerical libraries; accepted CPU/wall <300s and RSS <1GiB.
Full input hash/identity must remain unchanged, with no WAL/SHM/journal.
Expected current D4 result is the complete `transport_only` report with
`status: incomplete`, unchanged D1/D3 limitation strings and empirical false.
This report is not empirical approval of an injury/fatigue betting model.

Latest saved actual input:
`/var/lib/betboy-live-backup-_04ttiwe/context-current.db`, 253333504 bytes,
99521 contents/receipts, 28 artifacts, 26 snapshots, 2 manifests, 0 rollbacks;
SHA256 `ced192e70a7240606906087df08b8e82be227189f06728e7c62dfb3b186d2f05`.
Before deployment, a fresh backup must establish whether those actual counts
and bytes are still current; an older input cannot stand in for a changed one.

All native Task8 processes are finished. Main/GitHub main/VPS are still
`2dd1116b68f3d94e9c24338c6c9dff9b01799221`. The installed updater is still
`74b1c4b1aa88788f6a8e1050905215953b5938faad0009719aa15164a494b78f`.
Fresh read-only checks confirmed app/Caddy, seven scheduled timers and both
HTTP health endpoints. The existing `betboy-tennis.service` failed state was
not cleared and is not represented as fixed. Timer health is not worker health.

No main push, updater replacement, app deployment, key/marker/data edit,
history pruning, cap increase or change to Cricket/A0/P4b3 has occurred here.

## Existing Tennis worker failure — read-only diagnosis

The fixed-pattern journal reader inspected only `betboy-tennis.service` on
11September2026 and emitted no raw journal text, secrets or URLs. Ordinary
SSH UID1000 sees no system-unit journal entries; the same bounded read with
existing sudo journal access returned26lines and these concrete outcomes:

- State-Rebuild: rc1 after17seconds.
- Tages-Scan: timeout after900seconds.
- Pipeline end: both scan and overall failed.

A second closed-pattern read narrows the rebuild result: ATP
`retained_fresh` with no error, WTA `failed` with `error_type=HTTPError`,
overall `REFRESH_PARTIAL`. It does not disclose or establish the HTTP URL,
status code or upstream cause; do not invent one or call the failure fixed.

No service was restarted/reset and no timeout was changed. This confirms a
real worker failure, not merely an obsolete systemd flag. A successful D4
checking repair or healthy timer alone does not establish a successful daily
Tennis refresh/scan. That claim requires its own actual completed worker run.
