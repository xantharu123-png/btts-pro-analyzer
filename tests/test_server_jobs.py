...
def test_polling_timers_keep_a_persistent_calendar_trigger():
    ...
    for timer_path in systemd.glob("betboy-*.timer"):
        service_path = timer_path.with_suffix(".service")
        service = service_path.read_text(encoding="utf-8")
        timer = timer_path.read_text(encoding="utf-8")
        if "Type=oneshot" in service:
...
def test_root_installers_pin_every_systemd_unit_to_reviewed_bytes():
    ...
    assert "reviewed_timer_target_sha256" not in updater
    ...
    for timer_name, (fixture, previous_hash) in final_timer_fixtures.items():
        assert fixture.read_bytes() == (systemd / timer_name).read_bytes()
        final_hash = hashlib.sha256(fixture.read_bytes()).hexdigest()
        assert updater.count(final_hash) == 1
        assert bootstrap.count(final_hash) == 1
        assert previous_hash not in updater
        assert previous_hash not in bootstrap
...
def test_backup_stage_dumpability_fix_is_strictly_pinned_after_transition():
    root = Path(__file__).resolve().parents[1]
    updater = (root / "deploy" / "update_server.sh").read_text(encoding="utf-8")
    bootstrap = (root / "deploy" / "bootstrap_server.sh").read_text(
        encoding="utf-8"
    ...
    assert repaired_hash == (
        "1441158c542e97a19b193fa0cd091b645ec6442d6d8157f1d4fceabbba72b026"
    )
    assert updater.count(repaired_hash) == 1
    assert bootstrap.count(repaired_hash) == 1
    assert "b11704036e7a6f2302970a12395cd120b4ec26c76eb60f63618896f5bef85e6d" not in updater
    assert "b11704036e7a6f2302970a12395cd120b4ec26c76eb60f63618896f5bef85e6d" not in bootstrap
    assert "reviewed_backup_stage_target_sha256" not in updater
    assert "target_backup_stage_helper_sha256" not in updater

    validate = _shell_function(updater, "validate_trusted_backup_stage_helper")
    assert '"${actual}" == "$(expected_backup_stage_helper_sha256)"' in validate
