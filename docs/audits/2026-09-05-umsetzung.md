...
## Zusätzlich im echten Produktionslauf gefunden

Der erste geprüfte Code-Release `df8acc58d17aaef19fa5609da70d62a447e43be6` erzeugte am 7. September 162 Prognosen (158 Fußball, 4 Tennis), erfasste alle ohne Ablehnung in der Evidenzdatenbank und aktualisierte 21 Tennisrevisionen. Drei als Betriebsfehler gezählte Sachverhalte wurden dabei präzisiert: unbehandelte alte Tenniszeilen, bereits isolierte mehrdeutige historische Abrechnungsziele und der nicht konfigurierte Cricket-Zugang. Keiner davon rechtfertigt erfundene Werte oder eine unbemerkte Abrechnung. Vergangene Legacyzeilen bleiben unverändert; Quarantäne bleibt ungelöst/diagnostiziert; Cricket bleibt ausdrücklich eine Datenlücke. Auf dem VPS sind weder `RAPIDAPI_KEY` noch `CRICKET_API_KEY` eingerichtet (nur Vorhandensein geprüft, keine Schlüssel ausgegeben).

**F12 – P1, im Browser reproduziert:** Der bisherige Reader verwarf das gesamte Tagesdokument, sobald ein einziges darin enthaltenes Spiel seit der Veröffentlichung begonnen hatte. Somit waren trotz 162 berechneter Prognosen null Karten sichtbar. Der Korrekturvertrag lautet: erst alle ursprünglichen Daten einschließlich abgelaufener Zeilen gegen den Veröffentlichungszeitpunkt validieren, danach nur natürlich gestartete Spiele aus den drei sichtbaren Auswahlkollektionen herausfiltern. Verbleibende Spiele, Reihenfolge, Modellwerte, Preiszeitstempel und persistierte Datei bleiben unverändert. Schon bei Veröffentlichung ungültige oder manipulierte Daten dürfen dadurch nicht akzeptiert werden.
