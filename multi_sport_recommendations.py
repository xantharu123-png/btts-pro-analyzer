...
def build_probability_candidate(
    *,
    ...
    """Build a generic, price-independent candidate for another scanner.

    Every workspace uses the same candidate and price-gate contract. Invalid
    probabilities, haircuts, or an empty market identity fail closed.
    """
    normalized_blockers = [str(reason).strip() for reason in blockers if str(reason).strip()]
